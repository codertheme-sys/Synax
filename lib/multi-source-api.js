...
// Binance (ana) + CoinGecko (fallback) kombinasyonu
...
export async function getAllPricesFromMultipleSources() {
  ...
  try {
    ...
    // CoinGecko'dan altın ve fallback coin'ler (Binance başarısız olursa)
    let coinGeckoPrices = {};
    try {
    ...
    // Son fallback: Ortalama altın fiyatı
    if (!goldPrice) {
      goldPrice = {
        price: 2050,
        priceChange24h: 0,
    ...
    // CoinGecko'dan gelen ve Binance'de olmayan coin'leri ekle (fallback)
