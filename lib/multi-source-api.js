...
// Binance (kripto) + CoinGecko (altın + diğer) kombinasyonu

import { getAllBinancePrices } from './binance-api';
import { getAllCoinPrices } from './coingecko-api';
...
export async function getAllPricesFromMultipleSources() {
  ...
  try {
    ...
    // CoinGecko'dan altın ve diğer varlıklar (optional, Binance yeterli)
    let coinGeckoPrices = {};
    try {
      const cgPrices = await getAllCoinPrices();
      if (cgPrices && typeof cgPrices === 'object') {
        // CoinGecko formatını normalize et
        Object.entries(cgPrices).forEach(([symbol, data]) => {
          if (data && data.price) {
            coinGeckoPrices[symbol] = {
              price: data.price,
      ...
      console.warn('CoinGecko fetch failed, using Binance only:', cgError.message);
      // CoinGecko hata verirse devam et, Binance yeterli
    }

    // Altın fiyatını CoinGecko'dan al (pax-gold veya tether-gold)
    let goldPrice = null;
    try {
      // CoinGecko'da altın için token'lar kullanılıyor
      const goldUrls = [
      ...
      for (const url of goldUrls) {
        try {
          const goldResponse = await fetch(url);
          if (goldResponse.ok) {
            const goldData = await goldResponse.json();
            const tokenData = goldData['pax-gold'] || goldData['tether-gold'];
      ...
      console.warn('Gold price fetch failed:', goldError.message);
    }
    
    // Son fallback: Ortalama altın fiyatı (per oz)
    if (!goldPrice) {
      goldPrice = {
        price: 2050,
        priceChange24h: 0,
    ...
    // CoinGecko'dan gelen ve Binance'de olmayan coin'leri ekle
    Object.entries(coinGeckoPrices).forEach(([symbol, data]) => {
      if (!combinedPrices[symbol]) {
        combinedPrices[symbol] = data;
      }
    });
